...
M.general = {
  i = {
    ...
    -- Setup Ctrl D and Ctrl U with center
    ...
    -- Insert ? for Right + Shift + /
    ["<S-Up>"] = { "?", "Insert ?" },

    -- Insert tab space at the beginning of the line
    ["<C-]>"] = { "<Home><Tab><End>", "Insert tab at beginning of line" },
  },
  ...
  n = {
    ...
    -- Insert tab space at the beginning of the line
    ["<C-p>"] = { "<Home>d4l<Esc>w", "Insert tab at beginning of line" },
  ...
  v = {
    ...
    ["<leader>r"] = {
      function()
        -- Get the selected text from the visual mode
        vim.cmd 'noautocmd normal! "vy' -- Yank selected text to register v
        local old_word = vim.fn.getreg "v"
        local new_word = vim.fn.input "Replace selection with: "

        -- Ensure the selected text and replacement are both valid
        if old_word ~= "" and new_word ~= "" then
          vim.cmd(string.format("'<,'>s/\\V%s/%s/gc", vim.fn.escape(old_word, "\\"), new_word))
        else
          print "Replacement canceled."
        end
      end,
    ...
    ["<leader>rm"] = {
      function()
        ...
        -- Ensure the selected text and replacement are both valid
        if old_word ~= "" and new_word ~= "" then
          vim.cmd(string.format("'<,'>s/\\V%s/%s/g", vim.fn.escape(old_word, "\\"), new_word))
        else
          print "Replacement all canceled."
        end
      end,
      "Replace selected text without confirmation",
    },
